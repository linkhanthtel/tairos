...
function ToDoLists() {
  ...
  return (
    <div className='bg-white h-screen flex flex-col items-center text-center'>
      <h1 className='text-3xl my-2 text-blue-500'>To Do Lists App</h1>
      {/* <span className='text-md my-5 text-red-500'>* Since this is hosted on free platform, your changes may not be saved.</span> */}
      <div>
        <input type="text" className='w-96 p-3 my-10 border-2 border-blue-500' value={tasks} onChange={e => setTasks(e.target.value)}/>
        <button className='p-3 mx-2 bg-blue-500 text-white' onClick={() => addTasks()}>Add Tasks</button>
      </div>

      <div className='w-full flex justify-center text-center text-slate-950'>
        <ul className='w-[500px]'>
          {toDoLists.map(newtask => 
            <li className='flex justify-between items-center bg-blue-500 hover:bg-blue-600 text-white my-3 px-3 py-3'>
